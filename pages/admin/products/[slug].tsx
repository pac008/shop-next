...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onNewTag = () => {
    ...
    const newTag =
      newTagValue.trim().replaceAll(" ", "").toLocaleLowerCase() || "";
    if (!newTag || currentTags.includes(newTagValue)) {
      setNewTagValue(newTagValue.trim().replaceAll(" ", ""));
      return;
    }
  ...
  const onDeleteTag = (tag: string) => {
    const updatedTags = getValues("tags").filter((t) => tag !== t);
  ...
  const onSubmit = async (form: FormData) => {
    ...
    try {
      ...
      console.log(data);
      if (!form._id) {
        // TODO poner "Guardado" en el botón
        return replace(`/admin/products/${form.slug}`);
      }
      setIsSaving(false);
    } catch (error) {
      console.log(error);
